...
**v0.0.6.** Being built test-first. `AreaHits` and `HitPlane` are complete, and `FiringCadence`'s
aim gate with them; **its timing is still a stub**. `dotnet test` reports 17 failures and 46 passes,
and that is the intended state, not a broken build.
...
| `FiringCadence` | Shot interval, burst pattern, and the aim tolerance a weapon may fire within. | 🟡 aim gate done; timing still red |
